"""Declarative analyst execution plan.

Replaces the seven if-else branches that used to live inline in
``GraphSetup.setup_graph``. Each analyst entry pins the LangGraph node
labels for its (agent → tool → clear) triplet plus the state key it
writes its report to. New analysts get added by registering a single
``AnalystNodeSpec``; the orchestration code stays untouched.

``concurrency_limit`` is propagated through the plan so a future
fan-out implementation can read it without having to plumb it through
multiple call sites — today the analyst chain is still wired
sequentially so this field is informational only.
"""
...
# Seven A-share analysts (four upstream + three fork-specific). The wire
# keys are stable identifiers used in saved configs, conditional logic
# method names (``should_continue_<key>``), and tool-node lookups.
ANALYST_NODE_SPECS: Dict[str, AnalystNodeSpec] = {
    ...
    "social": AnalystNodeSpec(
        ...
        # underlying implementation is the redesigned sentiment_analyst
        # (pre-fetch + single LLM call, no tool-calling loop).
        key="social",
    ...
    "hot_money": AnalystNodeSpec(
        key="hot_money",
        agent_node="Hot_money Analyst",
        clear_node="Msg Clear Hot_money",
...
class AnalystWallTimeTracker:
    """Lightweight per-analyst wall-time tracker.

    Provided for diagnostic UIs (CLI / Streamlit) that want to surface how
    long each analyst took without instrumenting the graph itself.
    """
...
def sync_analyst_tracker_from_chunk(
    ...
    """Inspect a LangGraph stream chunk and update the tracker accordingly.

    A chunk that contains an analyst's ``report_key`` means that analyst
    has finished; the first analyst without a report is the active one.
    """
